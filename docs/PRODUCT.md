...
- The normal network-facing Homer agent must not run as root or through sudo. The tested Bazzite background user-service context requires the optional fixed-purpose power helper selected in D-051; SteamOS must still validate stock authorization before deciding whether it needs the same path. Declining or losing the helper leaves unavailable power controls disabled.
- Any elevated power or future recovery helper must not accept network requests directly and must expose only its fixed action allowlist through narrow local IPC. Helper installation, replacement, and removal are deliberate local maintenance actions, never phone requests or automatic updates. Homer must not install a blanket polkit rule.
