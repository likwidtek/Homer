...
- **Status:** Superseded by D-039, D-042, and D-058
- **Date:** 2026-08-01
...
## D-051 — Selected Bazzite v0.1 power-helper boundary

- **Status:** Accepted
- **Date:** 2026-08-08
- **Decision:** Because the tested Bazzite background user-service context cannot reach stock interactive logind authorization and fixed power actions remain accepted v0.1 scope, Homer will pursue D-048’s optional, removable, fixed-purpose local power helper for that configuration. Installation, replacement, and removal require deliberate local elevation and cannot be requested by a phone. The rootless agent sends one fixed immediate `sleep`, `restart`, or `shutdown` operation through authenticated local Unix IPC to an on-demand privileged service; the helper has no network listener, shell, caller-supplied arguments, generic subprocess path, configuration-selected commands, or polkit rule. Phone confirmation and the restart/shutdown countdown, including cancellation, finish in the unprivileged agent before dispatch, so the privileged interface has no cancellation operation. This refines D-048’s reference to required cancellation: v0.1 satisfies it before dispatch rather than through a fourth privileged action. The helper checks the configured user through kernel peer credentials, uses a fixed versioned protocol, invokes the operating system’s logind interface without interactive authorization, and fails closed when absent, unhealthy, or incompatible. Helper maintenance is never automatic or remotely initiated. Store inclusion requires explicit acceptance by Decky maintainers; an artifact that cannot include the helper disables the unavailable power controls.
- **Rationale:** Disabling power on the tested required Bazzite target would reduce accepted v0.1 scope. A one-operation local privilege boundary keeps the network service rootless, avoids the prohibited application-supplied polkit rule, preserves system inhibitor handling, and is smaller and more reviewable than a general privileged backend. The residual risk that another process running as the configured local user can request a power action is explicit and bounded to those three actions.

## D-052 — Minimal Python implementation direction for the power helper

- **Status:** Accepted
- **Date:** 2026-08-09
- **Decision:** Implement D-051's privileged boundary first as a small Python-standard-library companion to the Python agent, not as a second-language component. A root-owned systemd service accepts one strictly bounded, versioned JSON request over a filesystem-backed Unix stream socket from the configured Homer user, verified through socket permissions and kernel peer credentials. The complete request schema contains only the protocol version and one `sleep`, `restart`, or `shutdown` operation; it has a small fixed byte limit, rejects unknown, missing, duplicate, extra, malformed, trailing, or incompatible input, and returns only bounded result classes. The helper has no generic verb framework, network access, caller-supplied command arguments, shell, file-writing interface, sudoers fallback, or automatic/remote maintenance path. Absence or incompatibility disables the affected controls. This supersedes only D-051's assumption that the helper must call logind directly: Phase 0 must compare a fixed root-run operating-system command with direct logind D-Bus and select the smallest option that preserves required inhibitor and failure behavior. Its bounded process lifetime, activation/rate-limit values, exact framing limits, hardening, packaging, and SteamOS applicability also remain evidence-gated implementation-plan details.
- **Rationale:** The disposable Python helper already passed the local privilege-boundary probe, and no observed failure justifies Rust, a custom binary protocol, or another bespoke layer. Reusing the accepted language and standard Unix facilities reduces toolchain, packaging, review, maintenance, and contributor burden without widening the three-action privilege boundary. A more complex implementation remains available only if a concrete validation failure demonstrates its need.

## D-053 — Fixed logind dispatcher and provisional helper service policy

- **Status:** Accepted
- **Date:** 2026-08-09
- **Decision:** D-052's helper uses three immutable, root-run `busctl` argument mappings to the documented `org.freedesktop.login1.Manager` methods `Suspend(false)`, `Reboot(false)`, and `PowerOff(false)`. It invokes the system bus with interactive authorization disabled, uses no shell or caller-supplied argument, does not use inhibitor-skipping flags, and has no alternate command or fallback dispatcher. This preserves logind as the operating-system policy and inhibitor boundary without adding a Python D-Bus dependency. The local protocol accepts exactly one EOF-delimited UTF-8 JSON request per Unix-stream connection, limits requests to 128 bytes and responses to 64 bytes, and applies a one-second request-read deadline. Socket activation starts a service that handles one request and exits. The provisional service policy permits three activations per 60 seconds through the service start limiter, gives the fixed `busctl` call a five-second timeout, and gives the helper a ten-second total runtime ceiling; Phase 0 may refine those timing values if harmless or separately approved real-sleep evidence shows they break correct logind behavior. The helper writes no audit files and sends journald only fixed operation/result classes without raw payloads, credentials, or caller content. Bazzite installation, replacement, and removal are deliberate elevated local operations, never rpm-ostree layering, phone requests, remote maintenance, or automatic updates. SteamOS applicability and version-dependent inhibitor behavior remain qualification requirements.
- **Rationale:** Direct fixed `busctl` calls make the three privileged effects more explicit than a broader `systemctl` command path while retaining the platform's documented power-management interface. The tested Bazzite systemd 258 host enforces ordinary block inhibitors for privileged and unprivileged callers; older systemd behavior must not be assumed without testing. Tight framing, time bounds, rate limiting, and class-only system logging constrain abuse and review surface. A same-user process can exhaust the three attempts and temporarily disable power controls for up to one minute, which is an accepted fail-closed availability tradeoff rather than an authority expansion.

## D-054 — Independently recoverable power-helper lifecycle

- **Status:** Accepted
- **Date:** 2026-09-05
- **Decision:** Treat the optional power helper as a deliberately installed persistent operating-system component, not a process that removes or replaces itself after a power action. Power dispatch and helper maintenance are separate privilege paths: the helper exposes only D-053's three fixed power operations, while a locally initiated elevated maintenance command is solely responsible for installation, replacement, rollback, and removal. The agent, paired phones, remote network requests, automatic updates, and the power helper itself cannot invoke helper maintenance. Install complete, root-owned, ordinary-user-read-only versions in a persistent host-integration directory; select one through an atomically replaced `current` link and retain one complete `previous` version for rollback. Systemd unit links resolve through the selected complete version. Bazzite uses the validated non-image-layered `/var/opt/homer/power-helper` persistent layout with its normal SELinux labeling; Homer does not weaken SELinux, add a custom SELinux policy, apply an ad hoc persistent relabel, or modify the operating-system image for this purpose. Systemd manages mutable locks and other ephemeral helper coordination state under `/run`, never inside the read-only persistent versions. A maintenance transaction verifies the signed local bundle and exact hashes, installs and validates a complete candidate before activation, prevents new requests and refuses replacement during an active dispatch, atomically selects the candidate, reloads systemd, performs a harmless compatibility check, and restores the prior selection if activation or health validation fails. Interrupted maintenance must leave a complete old or new selection and be safely reconcilable when rerun. Removal is a separate idempotent elevated local operation that disables the socket, removes unit links, reloads systemd, and removes helper versions and runtime state; it never relies on code running after sleep, restart, or shutdown. A root-owned non-secret manifest exposes only the helper version, protocol compatibility, and installed-file hashes so the rootless agent can fail closed and disable power controls without probing through a real power action. Agent and Store updates never replace the helper. If a future agent is incompatible with the installed helper, automatic activation is deferred until the owner performs the deliberate local helper upgrade. Store inclusion of the helper remains contingent on explicit Decky-maintainer acceptance; otherwise the Store artifact disables helper-dependent power controls. SteamOS first tests the same `/var/opt/homer/power-helper` layout but receives no compatibility claim until direct evidence or an authoritative platform guarantee establishes persistence across normal system updates, and direct testing confirms root-only ownership, executable mount and host-security labeling, hardened systemd execution, reboot persistence, atomic selection and rollback, and complete removal without disabling the read-only root or modifying the OS image. If that path fails, Architecture must approve an evidence-backed SteamOS-specific path; maintenance uses only an explicit supported-platform mapping and fails closed rather than searching for another executable location.
- **Rationale:** Restart and shutdown correctly terminate the dispatching process, so post-action cleanup cannot provide lifecycle safety. Complete version directories plus one atomic selection point make installation and rollback recoverable without a privileged updater daemon, maintenance listener, package layering, or self-modifying power service. Bazzite validation showed that a merely generic `/var` choice is insufficient: default `/var/lib` labeling denied execution, while `/var/opt` passed without weakening host security. The separate local elevation step is intentional friction at the privileged-code boundary while ordinary power use remains couch-friendly.

## D-055 — Bounded SteamOS Gaming Mode native-input spike

- **Status:** Accepted
- **Date:** 2026-09-05
- **Decision:** Do not reduce v0.1's SteamOS Gaming Mode pointer and scrolling scope after failure of the first rootless raw virtual-input route. Run one bounded two-candidate capability spike before revisiting scope. First inspect and, only when SteamOS exposes an existing rootless compositor connection, test its installed standard emulated-input client path for relative pointer, buttons, and scrolling. Reject that path if it requires recurring authorization, persistent host-policy weakening, a new privileged service, or an unapproved dependency installation. Second, establish the provenance and ordinary-baseline status of the installed input-routing service and inspect its live Gaming Mode local API. Test only an already available documented virtual-mouse output that does not enable input interception, grab or remap a physical controller, alter profiles, emulate a controller, start or reconfigure the service, or use network features. Do not repeat the failed raw virtual-input pointer route, change Steam Input layouts, disable the read-only root, modify SteamOS, add another privileged helper, or write production code. A viable candidate must work from the intended rootless user-service context, visibly deliver relative movement, left/middle/right clicks, and scrolling in the Steam Gaming Mode UI and one safe in-game target, require no per-session prompt, survive claimed Gaming/Desktop transitions, avoid physical-controller interference, and expose a stable reviewable platform interface. Batch owner-visible checks. If neither candidate satisfies the boundary, stop input-route experimentation and return to Architecture for an explicit choice between a visible SteamOS Gaming Mode limitation and reconsideration of the rootless input architecture; a third framework requires a new decision.
- **Rationale:** Pointer control is part of Homer's primary v0.1 job and SteamOS is a required initial target, while only one injection mechanism has failed. The installed standard compositor protocol is designed for controlled emulated input, and the existing routing service may expose a suitable output interface, so a narrow native-capability check is warranted before reducing scope. The ordered stop condition prevents that justification from becoming open-ended experimentation or controller interception.

## D-056 — Rootless native libei adapter spike for SteamOS

- **Status:** Accepted
- **Date:** 2026-09-05
- **Decision:** Before limiting SteamOS Gaming Mode pointer and scrolling, complete D-055's first candidate through one disposable, off-device-built native client of the installed libei runtime and the existing ordinary-user-accessible Gamescope EIS socket. This is a narrow reconsideration of the input adapter, not the Python network agent, its rootless boundary, or a third input framework. The probe uses libei's documented C client API at the minimum supported SteamOS runtime ABI, installs no compiler, headers, package, development tool, runtime library, service, or policy on SteamOS, and introduces no privilege. It connects only to a validated EIS socket in the graphical user's runtime directory, requests only relative pointer, button, and scroll capabilities, ignores unexpected capabilities, and performs no keyboard input, physical-controller access, interception, remapping, profile change, gamepad emulation, Steam configuration change, or network communication. Phase 0 uses fixed compiled synthetic events rather than a general agent-to-adapter command protocol. The candidate must be reproducibly built from reviewable source, declare and verify its dynamic dependencies, fail closed when its socket or required capability is absent, visibly deliver relative movement, left/middle/right clicks, and scrolling in Steam Gaming Mode and one safe in-game target, require no prompt or elevation, reconnect across claimed Gaming/Desktop transitions, leave physical controller behavior unaffected, meet bounded startup/resource expectations, and clean up completely. Passing returns to Architecture for an explicit production-backend, IPC, packaging, ABI, dependency, and Store-review decision. Failure ends input-route experimentation and triggers an explicit visible SteamOS Gaming Mode limitation unless the owner separately reopens a privileged or controller-integrated architecture.
- **Rationale:** SteamOS exposes the appropriate rootless compositor endpoint, but supplies no usable client tool or supported Python binding, while raw virtual mouse and scrolling events failed. A small process-isolated client of the platform's documented C API is narrower and more reviewable than reimplementing its asynchronous wire protocol in Python, changing the main agent language, installing a host toolchain, or weakening the rootless boundary. The added native artifact and ABI obligation are now justified by a concrete required-platform failure and remain validation-gated.

## D-057 — Visible SteamOS Gaming Mode system-menu scrolling limitation

- **Status:** Accepted
- **Date:** 2026-09-07
- **Decision:** Homer v0.1 accepts a narrow, visible limitation on the tested SteamOS Gaming Mode configuration: relative pointer movement and left, middle, and right clicks are supported through the validated rootless compositor path, and scrolling remains available for compatible games, but Homer does not claim scrolling support in Steam's Gaming Mode system interface. The client continues to send ordinary scroll input because the same path worked in the tested game; it does not attempt to infer whether Steam or a game has focus. Compatibility, setup, and status/help surfaces must disclose the Steam-interface limitation without repeated warning prompts. Use the Deck's normal controller or physical controls to navigate Steam menus when scrolling is needed. Do not retry raw virtual input, InputPlumber, another input framework, another libei event variation, a privileged input path, or controller-integrated input for this v0.1 gap. Production use of the small rootless native libei adapter remains subject to the final implementation plan's narrow IPC, reproducible build, pinned ABI/dependency, packaging, fail-closed, mode-transition, resource, and Decky Store review gates; D-057 authorizes no production code or broad SteamOS compatibility claim.
- **Rationale:** Phase 0 demonstrated rootless relative motion and all three mouse buttons in Steam Library and a tested game, normal physical-controller coexistence, and corrected discrete scrolling in that game, while the same scrolling did not visibly affect a clearly scrollable Steam Library target. Expanding privilege or controller integration for one system-menu behavior would materially increase security, packaging, maintenance, and Store-review risk. A precise limitation preserves Homer's core couch-control value and the working in-game path without hiding the tested boundary or starting open-ended input experimentation.

## D-058 — Validation-gated Noise profile and shared cryptographic core

- **Status:** Accepted
- **Date:** 2026-09-07
- **Decision:** Use Noise revision 34 profile `Noise_NNpsk0_25519_ChaChaPoly_BLAKE2s` as Homer's Phase 0 encrypted-channel candidate, with the rootless agent as Noise initiator and the phone browser as responder. A public random credential selector lets the agent choose the 256-bit pairing bootstrap, paired-phone credential, or one-time migration secret before it sends the first Noise message. The selector, Homer transport version, fixed initiator/responder roles, and exactly one purpose—pairing, paired session, or origin migration—must be included through one fixed, unambiguous, length-delimited prologue encoding; migration also binds its intended target origin. Both Noise handshake messages carry empty encrypted payloads. After the handshake, the browser sends an encrypted typed `ClientReady` containing the final handshake hash and responder role; the agent validates it before sending the corresponding encrypted `ServerReady` with the same hash and initiator role. No credential issuance or activation, pairing-state consumption, status disclosure, control-lease acquisition, update action, input, clipboard, power request, or other application action occurs before the authenticated ready exchange. The pairing comparison code is the zero-padded six-digit result of interpreting the first four handshake-hash bytes as an unsigned big-endian integer modulo 1,000,000. A newly approved phone credential remains pending and actionless until the browser's IndexedDB transaction commits, the browser acknowledges storage inside the encrypted pairing channel, and the agent confirms activation; a bounded pending recovery may finish only that activation after interruption. Ordinary sessions establish a fresh WebSocket and handshake after 24 hours or before either sending or receiving direction reaches `2^32` encrypted transport messages. Homer adds no custom in-session rekey mechanism.
- **Implementation boundary:** Phase 0 uses Snow `0.10.0` as one narrowly scoped Rust cryptographic core, compiled through PyO3 `0.29.2` for the Python agent and wasm-bindgen `0.2.128` for the locally served browser module, with getrandom `0.3.4` and its browser `wasm_js` backend. Cacophony `0.11.0` at commit `8ee9d41e34a1a596cfa3ab12aa4069ff87dc1247` is a test-only independent interoperability oracle and is never shipped. Each WebSocket binary message contains exactly one Noise handshake or transport message; compression, plaintext sensitive APIs, bearer-header authentication, protocol/algorithm negotiation, CDN assets, and third-party runtime code are absent. This supersedes only D-023's bearer-authorization mechanism; its local HTTP/WebSocket, trusted-LAN, no-public-exposure, and user-warning boundaries remain accepted. The shared core exposes only Homer's fixed handshake purposes, bounded encrypt/decrypt operations, handshake hash, and state closure—not a general configurable cryptographic framework. It runs without root, adds no user-installed compiler or runtime dependency, and is delivered as prebuilt signed native and browser artifacts inside the existing installation channels.
- **Risk and validation boundary:** Snow states that it has not received a formal audit and its current internal handshake/transport state does not guarantee immediate memory zeroization on destruction. Homer accepts those as explicit candidate risks, not as final production assurance. It erases buffers it controls, disables core dumps, bounds session lifetime, keeps the agent rootless, and makes no promise against a process already able to read its memory. Before production selection, locked native and WebAssembly builds, full dependency/license/vulnerability/manual review, official vectors, independent Cacophony interoperability, replay and malformed-state rejection, fuzzing, binary framing, ten-second handshake timeout, two-phase activation, and physical iPhone Safari and Android Chrome validation must pass. A material implementation defect, incompatible license, unresolved high/critical vulnerability, opaque dependency boundary, failed browser/runtime build, or inability to review the exposed module rejects the candidate rather than authorizing custom cryptography.
- **Rationale:** A unique 256-bit per-purpose PSK already supplies Homer's device authority, while `NNpsk0` adds fresh ephemeral keys and established authenticated transport without certificates or a custom handshake. One small source core for native Python and browser WebAssembly reduces cross-language protocol drift and review surface. Its extra compiled release artifacts are maintainer-controlled and add no installation step or privilege for users. The ready exchange, purpose binding, selector binding, and two-phase credential activation address concrete cross-purpose, partial-handshake, and interrupted-storage failures without growing into a general protocol layer.
