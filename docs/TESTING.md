...
If a required SteamOS Gaming Mode pointer or scrolling path fails, D-055 permits only the ordered native-capability spike defined there. Inspect the installed standard compositor emulated-input path first and use it only through an existing rootless connection. Then establish the installed input-routing service's provenance and ordinary-baseline status and inspect its live local API; test only an already available documented virtual-mouse output. Do not repeat the failed raw virtual-input route, install a dependency, start or reconfigure a privileged service, enable interception, grab or remap physical controls, modify profiles or Steam Input layouts, emulate a controller, use a network-input feature, weaken host policy, or modify SteamOS. Passing requires relative motion, left/middle/right clicks, and scrolling from the intended rootless service context in both the Gaming Mode UI and one safe in-game target, no recurring authorization, no physical-controller interference, mode-transition continuity, and a stable reviewable platform interface. Stop after these two candidates and return failures or boundary changes to Architecture before another input mechanism or a reduced compatibility claim.

D-056 permits one final continuation of the first D-055 candidate: a disposable native libei client built off the Steam Deck against the minimum supported installed runtime ABI. Do not install a host compiler, headers, package, development tool, runtime library, service, or policy. The probe source and build must be reviewable and reproducible; record its source hash, artifact hash, ABI target, dynamic dependencies, and sanitized runtime version. It may connect only to the validated ordinary-user Gamescope EIS socket, request only relative pointer, button, and scroll capabilities, ignore unexpected capabilities, and emit only fixed compiled synthetic test events. Validate fail-closed missing-socket and missing-capability behavior, startup/resource bounds, visible relative motion, all three buttons and scrolling in Gaming Mode and one safe in-game target, reconnect after claimed Gaming/Desktop transitions, unchanged physical-controller behavior, and complete process/artifact cleanup. Do not design a production IPC protocol, perform keyboard input through this adapter, access or remap controllers, emulate a gamepad, change Steam or host configuration, use elevation, or add production code. If the adapter passes, return to Architecture before selecting a production dependency or packaging model. If it fails, stop input-route testing and return for the accepted limitation decision; no additional mechanism is authorized.

D-057 records the resulting limitation decision. The tested rootless compositor path qualified SteamOS Gaming Mode relative movement and all three buttons in Steam Library and one game, plus scrolling in that game, but did not qualify scrolling in Steam's system interface. Phase 0 input-route experimentation stops here. Do not repeat or vary the rejected routes. Later implementation-plan validation of the retained native adapter must cover its narrow IPC, reproducible build, ABI and runtime dependency checks, packaging, fail-closed behavior, resource bounds, claimed mode transitions, controller coexistence, and Store-review evidence; those gates do not reopen the Steam-interface scrolling claim.

The candidate passes only for configurations where accepted functionality works within the documented rootless-agent boundary. A power-helper path first uses a harmless non-power backend to demonstrate deliberate local installation, the D-052 Python-standard-library implementation direction, small strictly validated versioned JSON over a filesystem-backed Unix stream socket, configured-user peer validation, rejection of other users and unknown, missing, duplicate, extra, malformed, trailing, oversized, and version-mismatched input, the exact three-action interface, no network listener, no polkit rule or sudoers fallback, no generic verb framework, no arbitrary command, file-writing interface, or privileged cancellation path, service hardening, fail-closed agent behavior, audit behavior, and clean removal. A harmless comparison must then determine whether a fixed root-run operating-system command or direct logind D-Bus is the smallest path that preserves required inhibitor and failure behavior. Only after that boundary passes may a separately approved and immediately warned real power test validate the selected call and actual inhibitor behavior.

D-054 lifecycle validation must use simulated dispatch and demonstrate a persistent non-image-layered root-owned layout, complete version staging, signed-manifest and exact-hash verification, atomic selection, one retained rollback version, harmless compatibility detection, automatic rollback after failed activation or health validation, recoverable interruption before and after selection, refusal to replace during an active dispatch, systemd reload/recovery behavior, fail-closed agent-visible incompatibility, and idempotent complete removal. On Bazzite, validate `/var/opt/homer/power-helper` with the host's normal SELinux policy and systemd-managed mutable coordination state under `/run`; `/var/lib` is not an accepted executable-helper fallback, and testing must not weaken SELinux, persist an ad hoc relabel, or modify the image. SteamOS must first test the same `/var/opt` layout and cannot pass until the location is root-only, executable under the actual mount and host-security policy, usable by the hardened service, persistent across reboot and a normal OS update by direct evidence or authoritative guarantee, compatible with atomic selection/rollback, and completely removable without disabling the read-only root or modifying the OS image. A different SteamOS path requires a new evidence-backed Architecture decision; automatic path searching fails the candidate. Restart or shutdown must not be part of this lifecycle probe: no success or cleanup condition may depend on code running after a power action. Automatic, agent-triggered, phone-triggered, remotely initiated, or helper-self-initiated maintenance fails the candidate. Any one-time setup or privileged probe must be identified precisely, recorded under device-change control, and separately approved before it changes the machine.
