...
Build to maximum Decky Plugin Store safety compliance even while initial releases are delivered through GitHub. Keep code concise, explainable, documented, tested, and manually reviewable. Prefer the smallest complete, reviewable change consistent with accepted requirements. Before adding code, configuration, an abstraction, or a dependency, confirm a present need; reuse existing project patterns, then standard-library or native-platform capabilities where sufficient. Avoid speculative scaffolding, and fix defects at their narrowest correct shared cause. Do not reduce required security, privacy, accessibility, data-loss safeguards, compatibility, validation, or explicit scope; state a deliberate limitation and its evidence-based upgrade trigger when it would otherwise be unclear. Security sensitivity alone does not justify a second implementation language, custom protocol, or bespoke layer: identify the concrete failure of the simpler same-stack or standard-platform option before escalating complexity. Do not add arbitrary remote code installation, controller-input hijacking, or broad destructive behavior. The only permitted Homer-managed update path is the explicitly accepted, narrowly bounded non-Store mechanism documented in `docs/SECURITY.md`; Store installations remain Decky-managed.
